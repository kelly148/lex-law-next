 ...
 * LegacyMatterView
 *
 * The v2.3 single-document-per-phase matter view.
 * Extracted from MatterPage.tsx with no functional changes.
 * Rendered by MatterPage when workflowModelVersion === 2.
 *
 * v2.4.2: This component is intentionally unchanged. It is the
 * authoritative v2.3 UI for model-2 matters.
 */
...
import {
  ...
  CircleDot, CircleSlash, Pencil, Check, X,
} from "lucide-react";
...
interface LegacyMatterViewProps {
  matterId: string;
}

export default function LegacyMatterView({ matterId }: LegacyMatterViewProps) {
    ...
    <div className="container py-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
            ...
            {editingClientName ? (
              <div className="flex items-center gap-2 mt-1">
                ...
                <Button
                  ...
                  disabled={!clientInput.trim() || updateClient.isPending}
              ...
              <div className="flex items-center gap-1.5 group mt-0.5">
                <p className="text-sm text-muted-foreground">
                  {matter.clientName || "No client name"}
                </p>
                <Button
                  size="icon" variant="ghost"
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() => setShowFactChange(true)}
          >
            Fact Change
          </Button>
        </div>
      ...
      {showFactChange && (
        <FactChangePanel
          ...
          onCreated={() => { setShowFactChange(false); }}
        />
      )}
