...
// Note: Provider-specific master prompts (identity, rules, style, firm context) are in
// server/masterPrompts.ts and are automatically prepended by the LLM client.
// The phase prompts below contain only the task-specific instructions for each phase.

export const PHASE_PROMPTS: Record<PhaseName, PhasePrompt> = {
  intake: {
    system: `You are performing Client Intake analysis. Review the provided source materials and extract all relevant facts, parties, dates, financial figures, and legal issues. Identify any inconsistencies or missing information. Flag items that require attorney verification with [ATTORNEY REVIEW NEEDED].`,
  ...
  issues: {
    system: `You are performing Issue Reconciliation. Based on the intake analysis, identify and reconcile all legal issues. Cross-reference facts across all source documents to find discrepancies. Each issue should be categorized by area of law and priority.`,
  ...
  planning: {
    system: `You are performing Matter Planning. Create a comprehensive legal strategy and action plan based on the identified issues. Include timeline estimates, resource requirements, and risk assessments.`,
  ...
  engagement: {
    system: `You are drafting an Engagement Letter. This is a formal legal document that establishes the attorney-client relationship. It must include all required elements under the applicable Rules of Professional Conduct. Use [PLACEHOLDER: description] for any information that needs to be filled in by the attorney.`,
  ...
  memo: {
    system: `You are drafting a Client Advisory Memorandum. This document advises the client on their legal position, options, and recommended course of action. It should be thorough yet accessible to a non-lawyer audience.`,
  ...
  matrix: {
    system: `You are creating a Client Decision Matrix. This document presents the client's options in a structured comparison format to facilitate informed decision-making.`,
  ...
  agreement: {
    system: `You are drafting the Operative Agreement/Document. This is the final legal document that implements the chosen course of action. It must be precise, enforceable, and compliant with the applicable jurisdiction's law. Use [PLACEHOLDER: description] for any terms requiring attorney finalization.`,
